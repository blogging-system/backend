...
class PostRepository extends BaseRepository<any> {
	...
	async findOne(query: FilterQuery<any>): Promise<any> {
		// TODO: add Keywords
		return await this.model.findOne(query).populate("imageId tags series").lean();
	}
	...
	async findMany(query: FilterQuery<any>, limit = 10): Promise<any[]> {
		return await this.model.find(query).populate("imageId tags series").limit(limit).lean();
	}
}
