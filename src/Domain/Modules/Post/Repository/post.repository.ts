...
class PostRepository extends BaseRepository<any> {
	...
	async findOne(query: FilterQuery<any>): Promise<any> {
		return await this.model.findOne(query).populate("tags series keywords image").lean();
	}
	...
	async findMany(query: FilterQuery<any>, limit = 10): Promise<any[]> {
		return await this.model.find(query).populate("tags series keywords image").limit(limit).lean();
	}
